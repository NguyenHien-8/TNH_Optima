...
- Droplet Auto Detect uses the baseline as a geometric constraint: it masks substrate/reflection, rejects top-connected needles, requires two-sided baseline contact, and uses the selected baseline segment with an overhang-safe search margin. A permissive silhouette mask locates the cap, outward-normal Scharr-gradient refinement places samples on the optical interface with subpixel peak interpolation, and persistent two-sided contrast removes thin annotation/substrate branches before uniform arc-length sampling.
...
- File Editor and Droplet Analysis are independent top-level windows
  registered with MainView; the Analysis window uses `WA_DeleteOnClose` to
  release closed instances. Save Analysis Result uses a user-selected path
  and persists its last successful directory independently in SQLite.
...
4. Live capture/record emits `media_created`; a VideoEditor capture does so
   only when Save As targets the current Item's `Image/` folder. SideBar
   updates immediately, while `QFileSystemWatcher` still synchronizes external
   changes.
